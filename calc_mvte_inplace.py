...
# Syntax:		python3 calc_mvte_inplace.py WDIR
# Arguments:	WDIR: working directory to pull data from and save data to

# Description:	use IDTxl to calculate multivariate transfer entropy in networks
# Requirements:	python
# Notes:		
...
# save out timer data
tf = open(os.path.join(outdir, "te_multivar_runtime.txt"), "w")
...
save_dict = {'te_multivar': te_mtx}
scipy.io.savemat(os.path.join(outdir,'te_multivar.mat'), save_dict)
        
# save out binary results
weights = 'max_te_lag'
...
bin_dict = {'te_multivar_bin': np_mtx}
scipy.io.savemat(os.path.join(outdir,'te_multivar_bin.mat'), save_dict)
